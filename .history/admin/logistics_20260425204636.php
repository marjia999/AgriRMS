...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_resource'])) {
    ...
    // Get delivery details for fee
    $delivery_query = mysqli_query($conn, "SELECT delivery_fee, location_type FROM delivery WHERE id = $delivery_id");
    $delivery = mysqli_fetch_assoc($delivery_query);
    $delivery_fee = $delivery['delivery_fee'];
    
    // Get request details for delivery address
    $request_query = mysqli_query($conn, "SELECT delivery_address, delivery_district, delivery_upazila FROM service_requests WHERE id = $request_id");
...
// Get approved requests where payment is NOT pending (client has paid)
$approved_requests = mysqli_query($conn, "SELECT sr.*, u.full_name, u.email 
...
$active_deliveries = mysqli_query($conn, "SELECT sr.*, u.full_name, u.email, u.phone, r.name as resource_name, r.model, d.delivery_type, d.location_type
                                         FROM service_requests sr 
                                         JOIN users u ON sr.user_id = u.id 
...
<head>
    ...
    <script>
        function updateResourceAndAddress() {
            ...
            if (selectedOption.value) {
                const resourceId = selectedOption.getAttribute('data-resource-id');
                const address = selectedOption.getAttribute('data-address');
                const deliveryId = selectedOption.getAttribute('data-delivery-id');
                
                // Auto-fill resource
                if (resourceId) {
                    for(let i = 0; i < resourceSelect.options.length; i++) {
                        if(resourceSelect.options[i].value == resourceId) {
                ...
                // Auto-fill delivery transport
                if (deliveryId) {
                    for(let i = 0; i < deliverySelect.options.length; i++) {
                        if(deliverySelect.options[i].value == deliveryId) {
            ...
            <form method="POST">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="request_id" id="request_id" required onchange="updateResourceAndAddress()">
                            ...
                            <option value="<?php echo $row['id']; ?>" 
                                    ...
                                    data-delivery-id="<?php echo $delivery_id; ?>"
                    ...
                    <div class="form-group">
                        ...
                        <select name="resource_id" id="resource_id" required>
                            ...
                            <option value="<?php echo $row['id']; ?>">
                                <?php echo htmlspecialchars($row['name']); ?> (<?php echo htmlspecialchars($row['model']); ?>) - <?php echo $row['quantity']; ?> available
                            </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="delivery_id" id="delivery_id" required>
                            ...
                            <option value="<?php echo $row['id']; ?>">
                                <?php echo htmlspecialchars($row['delivery_type']); ?> (<?php echo $row['location_type']; ?>) - ৳<?php echo number_format($row['delivery_fee'], 2); ?>
                            </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <small style="color: #888;">Address will be auto-filled from client's request</small>
                    </div>
                </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo htmlspecialchars($row['delivery_type'] ?? 'N/A'); ?>
                                <br><small><?php echo htmlspecialchars($row['location_type'] ?? ''); ?></small>
                             </td
                            <td>

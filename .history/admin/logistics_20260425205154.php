...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_resource'])) {
    ...
    $delivery_query = mysqli_query($conn, "SELECT delivery_fee FROM delivery WHERE id = $delivery_id");
...
$approved_requests = mysqli_query($conn, "SELECT sr.*, u.full_name, u.email, r.name as resource_name, r.model, d.delivery_type, d.location_type, d.delivery_fee
    ...
    <script>
        function updateDetails() {
            ...
            if (selectedOption.value) {
                const resourceName = selectedOption.getAttribute('data-resource-name');
                const resourceModel = selectedOption.getAttribute('data-resource-model');
                const deliveryType = selectedOption.getAttribute('data-delivery-type');
                const deliveryLocation = selectedOption.getAttribute('data-delivery-location');
                const deliveryFee = selectedOption.getAttribute('data-delivery-fee');
                ...
                document.getElementById('resource_display').innerHTML = 
                    ...
                    '<span class="value">' + (resourceName || 'N/A') + ' (' + (resourceModel || 'N/A') + ')</span></div>';
                ...
                document.getElementById('delivery_display').innerHTML = 
                    ...
                    '<span class="value">' + (deliveryType || 'N/A') + ' (' + (deliveryLocation || 'N/A') + ') - ৳' + (parseFloat(deliveryFee).toLocaleString() || '0') + '</span></div>';
                ...
                <div class="form-group">
                    ...
                    <select name="request_id" id="request_id" required onchange="updateDetails()">
                        ...
                        <option value="<?php echo $row['id']; ?>" 
                                ...
                                data-delivery-type="<?php echo htmlspecialchars($row['delivery_type']); ?>"

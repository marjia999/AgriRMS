...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['make_payment'])) {
    ...
    if ($payment) {
        // Check if request is approved or delivered/returned
        ...
        if (!in_array($payment['request_status'], $allowed_statuses)) {
            ...
            // For Cash on Delivery, don't require transaction ID
...
// Get all payments for this user with request status
$payments_query = "SELECT p.*, sr.resource_id, sr.request_status, r.name as resource_name, r.model,
...
// Get pending payments where request is approved/delivered/returned
$pending_payments_query = "SELECT p.*, sr.request_status, sr.resource_id, r.name as resource_name, r.model,
...
// Get pending approval payments (requests not yet approved)
$pending_approval_query = "SELECT p.*, sr.request_status, sr.resource_id, r.name as resource_name, r.model
                          FROM payments p
                          LEFT JOIN service_requests sr ON p.booking_id = sr.id
                          LEFT JOIN resources r ON p.resource_id = r.id
                          WHERE p.user_id = $user_id 
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Pending Payments Section (Approved requests only) -->
        ...
        <!-- Pending Approval Section (Requests not yet approved) -->
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                switch($row['payment_method']) {
                                    case 'Cash on Delivery': $method_icon = '<i class="fas fa-money-bill"></i>'; break;
                                    ...
                                    default: $method_icon = '<i class="fas fa-credit-card"></i>';
                                }
                            ...
                            <td>
                                <?php
                                $req_status_class = 'status-pending';
                                if($row['request_status'] == 'Approved') $req_status_class = 'status-approved';
                                elseif($row['request_status'] == 'Returned') $req_status_class = 'status-paid';
                                ?>
                                <span class="status <?php echo $req_status_class; ?>"><?php echo $row['request_status'] ?? 'Pending'; ?></span>
                             </td

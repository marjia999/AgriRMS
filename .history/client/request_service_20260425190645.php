...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($quantity > $resource['quantity']) {
        ...
        if ($rental_duration == 'Weekly') {
            $total_cost = ($resource['weekly_rate'] ?? $daily_rate * 7) * $quantity;
        } elseif ($rental_duration == 'Monthly') {
            $total_cost = ($resource['monthly_rate'] ?? $daily_rate * 30) * $quantity;
        } else {
            $total_cost = ($days * $daily_rate) * $quantity;
        }
        
        $query = "INSERT INTO service_requests (user_id, resource_id, rental_duration, quantity, start_date, end_date, total_rental_cost, total_cost, delivery_address, delivery_district, delivery_upazila, request_status, payment_status, created_at) 
                  VALUES ('$user_id', '$resource_id', '$rental_duration', '$quantity', '$start_date', '$end_date', '$total_cost', '$total_cost', '$delivery_address', '$delivery_district', '$delivery_upazila', 'Pending', 'Pending', NOW())";
        
        if (mysqli_query($conn, $query)) {
            $request_id = mysqli_insert_id($conn);
            
            // Create payment record
            $payment_query = "INSERT INTO payments (user_id, booking_id, resource_id, payment_type, resource_cost, total_amount, paid_amount, payment_status, created_at) 
                              VALUES ('$user_id', '$request_id', '$resource_id', 'Rental', '$total_cost', '$total_cost', '0', 'Pending', NOW())";
                ...
                <form method="POST" id="requestForm">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="rental_duration" id="rental_duration" required onchange="calculateTotal()">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label><i class="fas fa-map-marker-alt"></i> Delivery District <span class="required">*</span></label>
                            <input type="text" name="delivery_district" id="delivery_district" required placeholder="e.g., Dhaka, Chittagong" oninput="calculateTotal()">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="delivery_upazila" placeholder="e.g., Gulshan, Motijheel" oninput="calculateTotal()">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="price-summary" id="priceSummary" style="display: none;">
                        ...
                        <div class="price-details">
                            ...
                            <div class="price-item">
                                ...
                                <div class="value" id="durationDisplay">—</div>
                            </div>
                        ...
                        <div class="calculation-formula" id="calculationFormula">
                            <i class="fas fa-chart-line"></i> Select resource and dates to see calculation
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateResourceDetails() {
            ...
            if (resourceSelect.value && selectedOption) {
                ...
                // Reset quantity to 1 and set max
                const quantityInput = document.getElementById('quantity');
        ...
        function calculateTotal() {
            ...
            const deliveryDistrict = document.getElementById('delivery_district').value;
            ...
            document.getElementById('durationDisplay').innerHTML = rentalDuration;
            
            let rate = 0;
            let rateText = '';
            
            if (rentalDuration === 'Daily') {
                rate = selectedResourceDaily;
                rateText = '৳ ' + selectedResourceDaily.toLocaleString() + '/day';
            } else if (rentalDuration === 'Weekly') {
                rate = selectedResourceWeekly;
                rateText = '৳ ' + selectedResourceWeekly.toLocaleString() + '/week';
            } else {
                rate = selectedResourceMonthly;
                rateText = '৳ ' + selectedResourceMonthly.toLocaleString() + '/month';
            }
            
            document.getElementById('displayRate').innerHTML = rateText || '—';
            
            if (startDate && endDate && selectedResourceDaily > 0 && quantity <= selectedResourceQuantity) {
                ...
                if (end >= start) {
                    ...
                    if (rentalDuration === 'Daily') {
                        total = days * selectedResourceDaily * quantity;
                        formulaText = '৳ ' + selectedResourceDaily.toLocaleString() + ' × ' + quantity + ' unit(s) × ' + days + ' days';
                    } else if (rentalDuration === 'Weekly') {
                        let weeks = Math.ceil(days / 7);
                        total = weeks * selectedResourceWeekly * quantity;
                        formulaText = '৳ ' + selectedResourceWeekly.toLocaleString() + ' × ' + quantity + ' unit(s) × ' + weeks + ' week(s)';
                    } else {
                        let months = Math.ceil(days / 30);
                        total = months * selectedResourceMonthly * quantity;
                        formulaText = '৳ ' + selectedResourceMonthly.toLocaleString() + ' × ' + quantity + ' unit(s) × ' + months + ' month(s)';
                    }
                    
                    document.getElementById('totalAmount').innerHTML = '৳ ' + total.toLocaleString();
                    document.getElementById('calculationFormula').innerHTML = '<i class="fas fa-chart-line"></i> ' + formulaText + ' = ৳ ' + total.toLocaleString();
            ...
            if (!startDate || !endDate || selectedResourceDaily === 0) {
        ...
        document.getElementById('rental_duration').addEventListener('change', calculateTotal);
        ...
        document.getElementById('delivery_district').addEventListener('input', calculateTotal);
